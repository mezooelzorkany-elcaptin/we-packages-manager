...
(function(){
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  const sync=()=>{const d=document.documentElement.classList.contains('dark-mode');if(i)i.textContent=d?'☀️':'🌙';document.body.classList.toggle('dark-mode',d)};sync();
  if(!b.dataset.themeBound){b.dataset.themeBound='1';b.addEventListener('click',()=>{const d=!document.documentElement.classList.contains('dark-mode');document.documentElement.classList.toggle('dark-mode',d);document.body.classList.toggle('dark-mode',d);localStorage.setItem(key,d?'dark':'light');localStorage.setItem('app_theme',d?'dark':'light');sync()})}
  // Administration navigation: invoices replaces the old packages shortcut.
  if(!/invoices\.html$/i.test(location.pathname)) document.querySelectorAll('a[href]').forEach(a=>{const h=(a.getAttribute('href')||'').split('?')[0].split('#')[0].toLowerCase();if(h==='packages.html'){a.href='invoices.html';a.textContent='🧾 الفواتير'}});
 });
})();
